...
class UnassignedVars:
    ...
    def __init__(self, select_criteria, csp):
        if select_criteria not in ['random', 'fixed', 'mrv']:
            print(
                "Error UnassignedVars given an illegal selection criteria {}. Must be one of 'random', 'stack', 'queue', or 'mrv'".format(
                    select_criteria))
    ...
    def insert(self, var):
        if not var in self.csp.variables():
            print(
                "Error, trying to insert variable {} in unassigned that is not in the CSP problem".format(
                    var.name()))
        else:
...
def bt_search(algo, csp, variableHeuristic, allSolutions, trace):
    ...
    if algo not in algorithms:
        print(
            "Error. Unknown algorithm heursitics {}. Must be one of {}.".format(
                algo, algorithms))
...
def FCCheck(cnstr, reasonVar, reasonVal):
    if cnstr.numUnassigned() != 1:
        print(
            "Error FCCheck called on constraint {} with {} neq 1 unassigned vars".format(
                cnstr.name(), cnstr.numUnassignedVars))
