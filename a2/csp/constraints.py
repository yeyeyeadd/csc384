        ...
        for x in qi.domain():
            for y in qj.domain():
                if x != y and abs(x - y) != abs(i - j):
                    satisfy.append([x, y])
        TableConstraint.__init__(self, name, scope, satisfy)
...
def findvals(remainingVars, assignment, finalTestfn,
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        return findvals(varsToAssign, [(var, val)], valsCheck, paritalValsCheck)
