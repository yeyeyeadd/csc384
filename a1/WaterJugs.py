...
class WaterJugs(StateSpace):
    ...
    def successors(self):
        ...
        if self.gal3 > 0:
            States.append(
                WaterJugs('Empty 3 Gallon', self.gval + 1, 0, self.gal4, self))
        if self.gal3 < 3:
            States.append(
                WaterJugs('Fill 3 Gallon', self.gval + 1, 3, self.gal4, self))
        if self.gal4 > 0:
            States.append(
                WaterJugs('Empty 4 Gallon', self.gval + 1, self.gal3, 0, self))
        if self.gal4 < 4:
            States.append(
                WaterJugs('Fill 4 Gallon', self.gval + 1, self.gal3, 4, self))
        if self.gal4 < 4 and self.gal3 > 0:
            maxpour = min(4 - self.gal4,
                          self.gal3)  # at most can only fill up 4 gallon
            States.append(
                WaterJugs('Pour 3 into 4', self.gval + 1, self.gal3 - maxpour,
                          self.gal4 + maxpour, self))
        if self.gal3 < 3 and self.gal4 > 0:
            maxpour = min(3 - self.gal3,
                          self.gal4)  # at most can only fill up 3 gallon
            States.append(
                WaterJugs('Pour 4 into 3', self.gval + 1, self.gal3 + maxpour,
                          self.gal4 - maxpour, self))
        return States
    ...
    def print_state(self):
        if self.parent:
            print(
                "Action= \"{}\", S{}, g-value = {}, (3gal, 4gal) = ({},{}), (From S{})".format(
                    self.action, self.index, self.gval, self.gal3, self.gal4,
                    self.parent.index))
        else:
            print(
                "Action=\"{}\", S{}, g-value = {}, (3gal, 4gal) = ({},{}), (Initial state)".format(
                    self.action, self.index, self.gval, self.gal3, self.gal4))
...
if __name__ == "__main__":
    ...
    print(
        "=========Test 4. Breadth first on unreachable goal with only path checking==")
    se.init_search(s0, waterjugs_goal_fn)
    final = se.search()[0]
    if final:
        final.print_path()
    ...
    waterjugs_set_goal(2, 1)
    print(
        "=========Test 5. Breadth first on unreachable goal with full checking==")
    se.init_search(s0, waterjugs_goal_fn)
    final = se.search()[0]
    if final:
        final.print_path()
    ...
    waterjugs_set_goal(2, 1)
    print(
        "=========Test 6. Depth first on unreachable goal with path checking==")
    se.init_search(s0, waterjugs_goal_fn)
    final = se.search()[0]
    if final:
        final.print_path()
